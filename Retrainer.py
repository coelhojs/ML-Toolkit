...
# Distorcoes para expansao da base de treinamento
...
class Retrainer:
    ...
    def __init__(self, workspace, steps = 4000): 
        self.Steps = steps
        self.Workspace = workspace
        #Setting training paths
        self.bottleneck_dir = f'{workspace}/bottleneck/'
        self.image_dir = f'{workspace}/images/'
        self.intermediate_dir = f'{workspace}/intermediate_files/'
        self.output_graphs_dir = f'{workspace}/output/'
        self.output_graph = f'{workspace}/output/output_graph.pb'
        self.output_labels = f'{workspace}/output/output_labels.txt'
        self.pre_trained_model_dir = f'{workspace}/pre_trained_model/'
        self.summaries_dir = f'{workspace}/summaries/'
    ...
    def create_image_lists(self, testing_percentage, validation_percentage):
        ...
        sub_dirs = [
            os.path.join(self.image_dir,item)
            for item in gfile.ListDirectory(self.image_dir)]
    ...
    def get_bottleneck_path(self, image_lists, label_name, index, category, architecture):
        ...
        Args:
            ...
            available number of images for the label, so it can be arbitrarily large.
            self.bottleneck_dir: Folder string holding cached files of bottleneck values.
            category: Name string of set to pull images from - training, testing, or
            validation.
        ...
        Returns:
            File system path string to an image that meets the requested parameters.
        """
        return self.get_image_path(image_lists, label_name, index,
    ...
    def create_model_graph(self, model_info):
        ...
        with tf.Graph().as_default() as graph:
            model_path = os.path.join(self.pre_trained_model_dir, model_info['model_file_name'])
    ...
    def maybe_download_and_extract(self, data_url):
        ...
        """
        dest_directory = self.pre_trained_model_dir
    ...
    def get_or_create_bottleneck(self, sess, image_lists, label_name, index, 
    ...
    def get_random_distorted_bottlenecks(self, 
        ...
        for unused_i in range(how_many):
            ...
            label_name = list(image_lists.keys())[label_index]
            image_index = random.randrange(Max_num_images_per_class + 1)
            image_path = self.get_image_path(image_lists, label_name, image_index, 
                                                                    category)
            if not gfile.Exists(image_path):
                tf.logging.fatal('File does not exist %s', image_path)
    ...
    def prepare_file_system(self):
        ...
        if intermediate_store_frequency > 0:
            self.ensure_dir_exists(self.intermediate_dir)
        return
    ...
    but found '%s' for architecture '%s'""",
            ...
            pre_trained_model_dir_name = 'mobilenet_v1_' + version_string + '_' + size_string
            model_file_name = os.path.join(pre_trained_model_dir_name, model_base_name)
            input_mean = 127.5
            input_std = 127.5
        else:
    ...
    def image_classification_retrainer(self):
        ...
        with tf.Session(graph=graph) as sess:
            ...
            # Merge all the summaries and write them out to the self.summaries_dir
            ...
            for i in range(self.Steps):
                ...
                if (intermediate_frequency > 0 and (i % intermediate_frequency == 0)
                        and i > 0):
                    intermediate_file_name = (self.intermediate_dir +
